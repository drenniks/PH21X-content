...
\end{center}
\textbf{Forces}
The forces in the $y$-direction of each coordinate system are balanced, so I will leave those off. Let the tension in the rope attached to $m_{1}$ be $F^{T}_{1}$, and the tension in the rope attached to $m_{3}$ be $F^{T}_{3}$
\begin{align*}
	m_{1}a\hat{x} & = \vec{F}^{net}_{1} = \left(F^{T}_{1} - m_{1}g\sin\theta_{1}\right)\hat{x} \\
	m_{2}a\hat{x} & = \vec{F}^{net}_{2} = \left(F^{T}_{3} - F^{T}_{1}\right)\hat{x} \\
	m_{3}a\hat{x} & = \vec{F}^{net}_{3} = \left(m_{3}g\sin\theta_{3} - F^{T}_{3}\right)\hat{x} \\
	(m_{1}+m_{2}+m_{3})a\hat{x} & = (-m_{1}g\sin\theta_{1}+m_{3}g\sin\theta_{3})\hat{x} \\
