...
\TeachingTips{\OverTip}{This problem is achievable for students, as they have been exposed to all of the necessary concepts. However, the lack of scaffolding (which is a rather healthy thing for them to experience) and length of the problem make it quite intimidating. For that reason, it may take a more hands-on approach from the instructor. First, go through the diagram with the entire class, asking them to identify the critical pieces of information that should be included based on what is being asked about in the subsequent parts. Part (a) requests a height of collision and a collision speed, so the diagram should at least depict the cliff, establish a coordinate system, and indicate the height of collision, as well as perhaps indicate initial and final speeds. Part (b) requests information about energy before and after the collision, and part (c) requests similar information about momentum, so our diagram should depict the situation (with relevant quantities indicated) at multiple relevant instants (at least the initial time, the time immediately before the collision, and the time immediately after). I chose to not depict the final time, as the exact location of the lower ball is not immediately apparent (though we will pretty quickly find out that it will return to the ground at that time). Be sure to define all of the symbols you devise on the board.}
...
\TeachingTips{\EnergyTip}{

Consider leading the students through the energy bar charts, asking them to help you set up the columns. They'll get the punchline of the activity, but they can still do the symbolic parts, now with the guidance of knowing the outcome.
}
\Solution{\CliffMidCollBSol}{

We can start by guiding ourselves with a graphical representation of how the energy in the dropped ball is distributed. We know that the height (and therefore the gravitational potential energy) must be the same at $t_{i}$ and $t_{f}$. Since the height right before and right after the collision is the same, the potential energies at $t_{bc}$ and $t_{ac}$ are equal to each other as well. These energies are smaller than the beginning and ending potential energies, as the collision height is lower than the top of the cliff.

The system we've defined (Earth and the ball) has no net external work from $t_{i}$ to $t_{bc}$, and no external work from $t_{ac}$ to $t_{f}$, so $E_{tot}(t_{i}) = E_{tot}(t_{bc})$ and $E_{tot}(t_{ac}) = E_{tot}(t_{f})$. The potential energy decrease from $t_{i}$ to $t_{bc}$ is therefore equal to the kinetic energy at $t_{bc}$, and the potential energy increase from $t_{ac}$ to $t_{f}$ is equal to the kinetic energy at $t_{ac}$.

\begin{center}
	\begin{tikzpicture}
		\begin{scope}[shift={(0,0)}]
			\draw[thick] (0,0) node[anchor=east] {$t_{i}$} -- (1.5,0);
			\draw[thick] (0.1,-0.1) -- (0.1,0) -- (0.4,0) node[anchor=north] {$K_{d}$} -- (0.7,0) -- (0.7,-0.1);
			%\filldraw[blue] (0.1,0) rectangle (0.7,);
			\draw[thick] (0.8,-0.1) -- (0.8,0) -- (1.1,0) node[anchor=north] {$U_{gd}$} -- (1.4,0) -- (1.4,-0.1);
			\filldraw[blue] (0.8,0) rectangle (1.4,1);
		\end{scope}
		\begin{scope}[shift={(2.5,0)}]
			\draw[thick] (0,0) node[anchor=east] {$t_{bc}$} -- (1.5,0);
			\draw[thick] (0.1,-0.1) -- (0.1,0) -- (0.4,0) node[anchor=north] {$K_{d}$} -- (0.7,0) -- (0.7,-0.1);
			\filldraw[blue] (0.1,0) rectangle (0.7,0.7);
			\draw[thick] (0.8,-0.1) -- (0.8,0) -- (1.1,0) node[anchor=north] {$U_{gd}$} -- (1.4,0) -- (1.4,-0.1);
			\filldraw[blue] (0.8,0) rectangle (1.4,0.3);
		\end{scope}
		\begin{scope}[shift={(7.5,0)}]
			\draw[thick] (0,0) node[anchor=east] {$t_{f}$} -- (1.5,0);
			\draw[thick] (0.1,-0.1) -- (0.1,0) -- (0.4,0) node[anchor=north] {$K_{d}$} -- (0.7,0) -- (0.7,-0.1);
			%\filldraw[blue] (0.1,0) rectangle (0.7,);
			\draw[thick] (0.8,-0.1) -- (0.8,0) -- (1.1,0) node[anchor=north] {$U_{gd}$} -- (1.4,0) -- (1.4,-0.1);
			\filldraw[blue] (0.8,0) rectangle (1.4,1);
		...
		\begin{scope}[shift={(5,0)}]
			\draw[thick] (0,0) node[anchor=east] {$t_{ac}$} -- (1.5,0);
			\draw[thick] (0.1,-0.1) -- (0.1,0) -- (0.4,0) node[anchor=north] {$K_{d}$} -- (0.7,0) -- (0.7,-0.1);
			\filldraw[blue] (0.1,0) rectangle (0.7,0.7);
			\draw[thick] (0.8,-0.1) -- (0.8,0) -- (1.1,0) node[anchor=north] {$U_{gd}$} -- (1.4,0) -- (1.4,-0.1);
			\filldraw[blue] (0.8,0) rectangle (1.4,0.3);
...
However, since that potential energy difference is the same across each of those time intervals, The kinetic energies at $t_{bc}$ and $t_{ac}$ must be equal. This is enough to conclude that the speed does not change after the collision, though the velocity does (it reverses direction, since the balls bounce off of each other).

To see this in symbolic form, let us write expressions for the energies of the dropped ball at all four times.
\begin{center}
...
\TeachingTips{\MomentumTip}{

Students need to recognize that, since part (a) gave them $v_{lbc}$ in terms of $v_{li}$, we need to find $v_{lbc}$ to get our answer. Leading the students through the momentum vector diagrams, asking them to spot your (intentional) mistakes, would be useful, and it would still leave the math for them.
}
...
In symbolic form, we can equate the initial and final momenta of the system, as momentum is conserved here:
\[
\begin{split}
...
\ProblemSub{\CliffMidCollD}{
(d) Perform sensemaking (use multiple techniques!) on your answer for initial speed of the launched ball.
}
\Solution{\CliffMidCollDSol}{
