...
There is no friction, and the normal force from the ice on the archer cancels with the force of gravity on the archer. As such, there will be no net force on the archer, and therefore no impulse.

For the arrow, we have to assume that there is no friction between the arrow and the archer's hands or the bow itself. We can perhaps also balance the vertical forces (gravity versus the normal force from the archer's hand), though exactly where the hand supports the arrow changes as it shoots out. However, even if we are unsure of the effect of this, an arrow can be expected to leave the bow fast enough that the time in which any net external force acts will be extremely brief, resulting in negligible impulse.

Without impulse, momentum is conserved.
}
